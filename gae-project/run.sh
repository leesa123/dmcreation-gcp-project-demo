...
#   ${12} ) You can select application environment. ex) flex, standard
#   ${13} ) Executes a script to handle the requests that match this URL pattern.
#   ${14} ) Methods to enforce security (HTTPS) on a URL.
#   ${15} ) URL prefix
#   ${16} ) Key value in manifest of the files stored in Google Cloud Storage
...
if [ $# -ne 16 ];
then
	echo -e '\e[31mYou must set sixteen arguments\e[0m'
	exit 1
fi
